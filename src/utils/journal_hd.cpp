...
std::string get_name_by_id(const uint64_t id)
{
    switch (id) {
        ...
        case actions::ACTION_DONE: return color::color(0,3,0) + "Action Done" + color::no_color();
        ...
        case actions::ACTION_ABORT_ON_ERROR: return color::color(5,0,0) + "Abort On Error" + color::no_color();
    ...
    for (auto entry : journal) {
        switch (entry.operation_name)
        {
            ...
            case actions::ACTION_DEALLOCATE_BLOCK:
            {
                std::stringstream ss;
                ss << time_to_hdtime(entry.timestamp) << ": " << get_name_by_id(entry.operation_name)
                    << " " << entry.operands.deallocate_block.where << ", block attr = "
            ...
            case actions::ACTION_ALLOCATE_BLOCK:
